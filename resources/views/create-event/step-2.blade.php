<x-dashboard-layout.minimal title="Create Event - Step 2" route="dashboard">
  <div class="w-full lg:w-3/5 mx-auto xl:px-12">
    ...
    <form action="{{route('post.event.step2')}}" method="POST" autocomplete="off">
      @csrf
      <div class="mb-4">
        ...
        <!-- <input type="text" placeholder="Ex: festa do pijama da Amanda..." class="border border-dark-100 rounded-lg h-10 py-4 px-4 text-xs w-full focus:border-primary-500 focus:text-primary-500 focus:bg-primary-200 focus:ring-0 placeholder:text-dark-200"> -->
      </div>

      <div class="mb-6">
        <div class="flex">
          <div class="flex items-center mr-4">
            <input checked id="gift_details_yes" onclick="openConfig(true)" type="radio" name="gift_details" @if(isset($event->gift_details) && $event->gift_details == 'yes') checked @endif value="yes" class="w-4 h-4  text-purple-600 rounded-full border-gray-300 focus:ring-0  ">
            <label for="gift_details_yes" class="ml-2 text-xs font-medium text-dark-500">Sim</label>
          </div>
          <div class="flex items-center mr-4">
            <input id="gift_details_no" onclick="openConfig(false)" type="radio" name="gift_details" @if(isset($event->gift_details) && $event->gift_details == 'no') checked @endif value="no" class="w-4 h-4  text-purple-600 rounded-full border-gray-300 focus:ring-0  ">
      ...
      <div data-accordion="collapse" class="mb-8">
        ...
        <div id="accordion-collapse" class="bg-dark-100 px-5 py-6 bg-opacity-20 rounded-lg hidden">
          <div class="mb-5">
            ...
            <textarea name="description" id="description" placeholder="Ex: estamos arrecadando esse valor para que possamos realizar o sonho da Amanda de ter uma guitarra elétrica." class="border border-dark-100 rounded-lg h-24 resize-none py-4 px-4 text-xs w-full focus:border-primary-500 focus:text-primary-500 focus:bg-primary-200 focus:ring-0 placeholder:text-dark-200">@if(isset($event->description)) {{ $event->description }} @endif</textarea>
          </div>

          <div class="mb-6 relative">
            <label for="" class="text-xs font-bold block mb-1.5 text-dark-500">Qual será o valor do presente?</label>
            <input name="goal" id="goal" type="text" placeholder="200,00" class="real border border-dark-100 rounded-lg h-10 py-4 pl-10 pr-4 text-xs w-full focus:border-primary-500 focus:text-primary-500 focus:bg-primary-200 focus:ring-0 placeholder:text-dark-200">
          ...
          <script>
            window.onload = function(){
              ...
              goalEL.value = "<?php if(isset($event->goal)) echo $event->goal; else ''; ?>";
          ...
          <div class="mb-5">
            ...
            <div class="flex">
              <div class="flex items-center mr-4">
                <input onchange="isDonation(true)" id="charity_yes" type="radio" @if(isset($event->donationInstitution) && $event->donationInstitution == 'yes') checked @endif name="donationInstitution" value="yes" class="w-4 h-4  text-purple-600 rounded-full border-gray-300 focus:ring-0  ">
                <label for="charity_yes" class="ml-2 text-xs font-medium text-dark-500">Sim</label>
              </div>
              <div class="flex items-center mr-4">
                <input checked onchange="isDonation(false)" id="charity_no" type="radio" @if(isset($event->donationInstitution) && $event->donationInstitution == 'no') checked @endif name="donationInstitution" value="no" class="w-4 h-4  text-purple-600 rounded-full border-gray-300 focus:ring-0  ">
          ...
          <div class="hidden" id="donation">
            ...
            <div class="mb-6">
              <select id="countries" name="institutionId" class="border border-dark-100 rounded-lg h-10 py-1 px-4 text-xs w-full focus:border-primary-500 focus:text-primary-500 focus:bg-primary-200 focus:ring-0 placeholder:text-dark-200">
                @foreach($institutions as $key => $institution)
                  <option @if(isset($event->institutionId) && $event->institutionId == $institution['id']) selected @endif value="{{ $institution['id'] }}">{{ $institution['name'] }}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="flex items-center">
              ...
              <div class="relative">
                <input id="donationPercentage" type="text" readonly name="taxCharity" value="@if(isset($event->taxCharity)) {{ $event->taxCharity }} @else 0 @endif" class="border border-dark-100 rounded-lg h-10 py-4 px-4 text-xs w-full mb-1 focus:border-primary-500 focus:text-primary-500 focus:bg-primary-200 focus:ring-0 placeholder:text-dark-200">
      ...
      <a href="../create-event" class="inline-flex justify-center items-center bg-white border border-danger-500 rounded-lg mr-4 w-auto px-6 h-10 text-sm font-bold text-danger-500 hover:bg-opacity-90">Voltar etapa</a>
      <button type="submit" class="bg-danger-500 rounded-lg w-auto px-6 h-10 text-sm font-bold text-white hover:bg-opacity-90">Continuar</button>
