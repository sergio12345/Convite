<x-dashboard-layout.minimal title="Create Event - Step 3" route="dashboard">
  <div class="lg:px-12 xl:px-28 lg:pt-12 lg:grid grid-cols-2 text-center lg:text-left relative overflow-x-clip">
    ...
    <div class="pr-16 text-center z-10 absolute top-56 left-8 w-full lg:static hidden lg:block show-on-template-select">
      <div id="templateSelected" class="@if(isset($event->image) && $event->image != null) hidden @endif md:shadow-lg shadow-primary-300 relative rounded-md py-28 sm:py-40 md:py-56 lg:py-60 xl:py-64 overflow-hidden -mx-5 md:mx-0 preview-placeholder">
      ...
      <img src="@if(isset($event->image) && $event->image != null) {{$event->image}} @endif" id="imageSelected" class="w-full h-auto rounded-sm preview-img shadow-lg shadow-primary-200">

    </div>

    <form action="{{route('post.event.step3')}}" method="POST" enctype="multipart/form-data" class="lg:pt-10">
      @csrf
      ...
      <a href="/create-event/2" class="inline-flex items-center justify-center bg-white border border-danger-500 rounded-lg mr-4 w-auto px-6 h-10 text-sm font-bold text-danger-500 hover:bg-opacity-90">Voltar etapa</a>
      
      <button type="submit" id="nextStep" class="bg-danger-500 rounded-lg w-auto px-6 h-10 text-sm font-bold text-white hover:bg-opacity-90">Continuar</button>
  ...
  <script>
    window.onload = function(){
      ...
      var type_template = "<?php if(isset($event->custom_template) && $event->custom_template != null) echo $event->custom_template; else null; ?>";
